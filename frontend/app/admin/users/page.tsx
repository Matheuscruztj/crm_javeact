...
export default function UsersPage() {
  ...
  if (loading) return <div className="p-6 text-muted-foreground" aria-busy="true">Loading users…</div>;
      ...
      {filtered.length === 0 ? (
        ...
        <div className="overflow-x-auto rounded-lg border">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((u) => (
                <tr key={u.id} className="border-t hover:bg-muted/25">
                  <td className="px-4 py-3 font-medium">{u.name}</td>
                  <td className="px-4 py-3 text-muted-foreground">{u.email}</td>
                  <td className="px-4 py-3">
                    <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${ROLE_COLORS[u.role] ?? "bg-muted"}`}>
                      {u.role}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <span className={`rounded-full px-2 py-0.5 text-xs ${u.active ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}>
