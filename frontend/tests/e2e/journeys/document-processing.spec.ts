...
test.describe("Document Processing: SSE progress → ANALYZED status", () => {
  test("should redirect unauthenticated user from documents page", async ({
    page,
  }) => {
  ...
  test("should connect to SSE stream endpoint", async ({
    adminLogin,
    page,
  }) => {
  ...
  test("should display processing progress indicator during upload", async ({
    ...
    const uploadArea = page
      .locator(
        "[data-testid='upload-zone'], [data-testid='upload-button'], input[type='file']",
      )
      .first();
