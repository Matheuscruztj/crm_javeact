...
class IntegrationControllerTest {
  ...
  @BeforeEach
  void setUp() {
    controller = new IntegrationController(dispatchWebhookUseCase);
  }
  ...
  void should_return200_when_dispatchSucceeds() {
    ...
    var request = new IntegrationController.WebhookDispatchRequest(
  ...
  void should_returnFalse_when_validateUrlIsLoopback() {
    ResponseEntity<Map<String, Object>> response = controller.validateUrl(
        TENANT_ID, Map.of("url", "http://127.0.0.1/internal"));
  ...
  void should_returnFalse_when_validateUrlIsPrivateNetwork() {
    ResponseEntity<Map<String, Object>> response = controller.validateUrl(
        TENANT_ID, Map.of("url", "http://192.168.1.100/api"));
