...
/**
 * Use case for activating a legal hold on a document.
 * Once a legal hold is active, the document cannot be archived or deleted.
 *
 * <p>Validates: P2.9 — Legal hold preventing archive/delete
 */
public class ActivateLegalHoldUseCase {

    private final DocumentRepository documentRepository;
    private final Clock clock;

    public ActivateLegalHoldUseCase(DocumentRepository documentRepository, Clock clock) {
        this.documentRepository = Objects.requireNonNull(documentRepository, "DocumentRepository must not be null");
        this.clock = Objects.requireNonNull(clock, "Clock must not be null");
    }

    /**
     * Activates a legal hold on the specified document.
     *
     * @param documentId the document identifier
     * @param tenantId   the tenant identifier
     * @param actorId    the user activating the hold (for audit purposes)
     * @return the updated document with legal hold active
    ...
    public Document execute(String documentId, String tenantId, String actorId) {
        ...
        document.activateLegalHold(clock.now());
